...
- **2.7 ✅ Media playability + play button** — _reframed_: do **not** port `SongHelper.GetPlayerInfos` regex into a server field. Platform detection is the video-player's job — delegate to `@mintplayer/video-player`'s `findApis(url, apis)` (a URL is playable iff some platform regex matches). `@mintplayer/ng-video-player@22` + core/plugins `@20` (youtube/vimeo/dailymotion/soundcloud/spotify/file); `VIDEO_PLAYER_PLUGINS` shared by `provideVideoApis` + a `MediaPlayabilityService`; a `media-player` column/detail renderer bound to `Medium.Value` (reordered first) shows a play-triangle when playable and opens an inline `<video-player>` overlay on click. Required a framework change (shipped + consumed): ng-spark's AsDetail sub-tables didn't consult `attr.renderer` — fixed in **`@mintplayer/ng-spark@22.0.3`** (Spark PR #182: AsDetail read cells resolve `col.renderer` → `columnComponent`). **Verified end-to-end:** Queen's Media grid shows the green play-triangle in the first column; clicking plays the YouTube medium inline (Bohemian Rhapsody). Applies to any Subject (Artist/Person/Song) with a playable medium, since `Media` is on the `Subject` base.
