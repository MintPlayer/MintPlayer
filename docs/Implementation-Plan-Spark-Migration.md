...
- **2.7 ◐ Media playability + play button** — _reframed_: do **not** port `SongHelper.GetPlayerInfos` regex into a server field. Platform detection is the video-player's job — delegate to `@mintplayer/video-player`'s `findApis(url, apis)` (a URL is playable iff some platform regex matches). **App side ✅** (committed): `@mintplayer/ng-video-player@22` + core/plugins `@20` (youtube/vimeo/dailymotion/soundcloud/spotify/file); `VIDEO_PLAYER_PLUGINS` shared by `provideVideoApis` + a `MediaPlayabilityService`; a `media-player` column/detail renderer bound to `Medium.Value` (reordered first) that shows a play-triangle when playable and opens an inline `<video-player>` overlay on click. **Blocked on a framework change ◐:** ng-spark's AsDetail sub-tables render cells via the `asDetailCellValue` pipe and don't consult `attr.renderer` (unlike query-list/po-detail/po-form), so the button can't appear in the Media grid yet. Handoff written (`~/.claude/pending-plans/ng-spark-asdetail-renderers-handoff.md`) to make AsDetail read tables honor registered renderers; lights up after that ng-spark patch ships + is consumed.
