...
- **1.6 ✅** PWA + SEO base. **DONE** — PWA via `ng add @angular/pwa` (`@angular/service-worker` + `ngsw-config.json`, branded `manifest.webmanifest` + icons, `provideServiceWorker` gated on `!isDevMode()` so it's inert in dev and served statically by the host in prod; prod build emits `ngsw.json` + workers; initial-bundle budget raised to 1MB/2MB). SEO base = `MintPlayerTitleStrategy` ("<page> | MintPlayer"), a default meta description, and site-wide JSON-LD (`WebSite` + `Organization`) via a tiny dependency-free `JsonLdService`. **NB:** `@mintplayer/ng-json-ld` (the D2 choice) is still on Angular `^18` peers (latest 18.0.0) — adopt once bumped to 22; native injection used meanwhile. Verified in the browser (title, description, manifest, theme-color, both JSON-LD blocks).

**Exit criteria:** Log in, manage MediumTypes in the admin UI, deploy via Docker; i18n + PWA + SEO scaffolding in place. _(Status 2026-06-07: 1.1–1.6 done; host+SPA+auth+MediumType slice running against RavenDB, full admin CRUD walkthrough green, PWA+SEO base in. Left to fully close Phase 1: `IEmailSender` (MailKit) and Docker Compose.)_

> **Packaging note (2026-06-07):** a `ProjectReference → PackageReference` swap for the Spark framework was attempted and reverted — the published NuGet feed is inconsistent (`MintPlayer.Spark` preview.34 depends on `MintPlayer.Spark.Abstractions >= preview.34`, but Abstractions is only published to preview.20 → NU1102). Stay on cross-repo `ProjectReference` until the framework republishes a coherent set (this validates D6's "ProjectReference now, NuGet at release").
